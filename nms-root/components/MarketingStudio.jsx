...
function GlobalMetaConnect({ globalMeta, onMetaChange }) {
  ...
  function connect(){ setConn(true); openMetaOAuth(pgs=>{ setPages(pgs); setConn(false); }); }
  function confirmPage(pg){ const m={ig:{userId:pg.instagram_business_account?.id||"",token:pg.access_token},fb:{pageId:pg.id,token:pg.access_token},nome:pg.name,allPages:pages.map(p=>({id:p.id,nome:p.name,igId:p.instagram_business_account?.id||"",token:p.access_token}))}; onMetaChange(m); setPages([]); }
    ...
    <div className="app">
      ...
      <div className="main">
        ...
        {view==="project"&&activeProj&&(
          <ProjectView project={activeProj} onUpdate={handleUpdate} onBack={handleBack} globalMeta={globalMeta}/>
        )}
      </div>
    </div>
  );
